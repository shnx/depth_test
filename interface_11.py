    ...
    try:
        while True:
            ...
            if grid_overlay_var.get():
                # Calculate points around the image
                points = []
                rows, cols = color_frame_data.shape[:2]

                # Generate points in a circular pattern (or evenly distributed)
                for i in range(9):
                    angle = i * (2 * np.pi / 9)
                    x = int((cols / 2) + (cols / 3) * np.cos(angle))
                    y = int((rows / 2) + (rows / 3) * np.sin(angle))
                    ...
                    # Draw circle or grid
                    cv2.circle(color_frame_data, (x, y), 5, (0, 255, 0), -1)

                    # Fetch and show depth value
