...
# Function to handle the robot's pose response and extract the z position
def get_robot_position():
    send_robot_command("get_actual_tcp_pose()\n")
    sleep(0.1)  # Allow some time for the robot to process the command
    try:
        response = robot_socket.recv(1024)
        # Check if the response is in binary or text format
        try:
            # Try to decode the response as a UTF-8 string
            response_text = response.decode('utf-8')
            print(f"Robot response: {response_text}")

            if "p[" in response_text:
                z_position = response_text.split(",")[2].strip(" ]")
                print(f"Current Z position of the robot: {z_position} meters")
            else:
                print("Unable to parse robot position from the response.")
        except UnicodeDecodeError:
            # If decoding fails, handle it as binary data
            print(f"Received binary data: {response}")
            # You can process the binary data based on the robot's protocol (e.g., extracting the z position)
            # Example: Assuming the z position is in a specific binary format or location
            # We will just print the raw binary response for now
            print(f"Raw binary response: {response}")
            # Add your logic here to handle the binary data if needed
    ...
    if results.detections:
        for detection in results.detections:
            ...
            if depth is not None:
                ...
                # Smoothing depth values (Exponential Moving Average)
                alpha = 0.1  # Smoothing factor
                smoothed_depth = (alpha * depth) + ((1 - alpha) * smoothed_depth if 'smoothed_depth' in locals() else depth)
                print(f"Smoothed Depth: {smoothed_depth} meters")

                # Print the current Z position of the robot
                get_robot_position()

                # If the face is within 60 cm (0.6 meters), move the robot closer or farther
                if smoothed_depth < 0.5:
                    print("Face is too close, moving robot away.")
                    z_move = 0.02  # Move away
                elif smoothed_depth > 0.6:
                ...
                command = f"movel(pose_trans(get_actual_tcp_pose(), {displacement}), a=0.3, v=0.3)\n"
